...
import { ArrowLeft, Check, Clock, Loader2, Minus, Plus } from 'lucide-react'
...
export function SignalDetail({
  ...
  const suggested = suggestedStake(prefs, price.book, price.odds, signal.fairOdds)
  const max = Math.max(1, Math.floor(limit ?? prefs.bankroll))

  const [stake, setStake] = useState(suggested)
  const [stakeText, setStakeText] = useState(String(suggested))
