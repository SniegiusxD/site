...
async function main() {
  ...
  try {
    ...
    for (const { id } of rows) {
      for (const table of ['bet_edit', 'billing_event', 'execution_event', 'book_request', 'feedback', 'book_limit_event', 'bankroll_entry', 'subscription', 'telegram_sent', 'telegram_preset', 'telegram_link_token', 'telegram_account', 'user_settings', 'user_bet', 'session', 'account']) {
